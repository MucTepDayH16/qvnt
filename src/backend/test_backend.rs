...
pub(crate) fn test_build(q_num: usize) -> TestBackend {
    let b0 = SingleThreadBuilder::default().build(q_num).unwrap();
    #[cfg(feature = "multi-thread")]
    let b1 = MultiThreadBuilder::default().build(q_num).unwrap();
...
mod tests {
    ...
    #[test]
    fn init() {
        let mut backend = test_build(4);
        backend.reset_state(0).unwrap();
        assert_eq!(backend.collect(), [&[C_ONE; 1][..], &[C_ZERO; 15]].concat());
    }
}
